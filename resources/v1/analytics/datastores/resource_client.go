...
func (c *Client) Create(request CreateRequest, reqModifiers ...RequestModifier) (types.GoogleCloudApigeeV1Datastore, error) {
	// build & send request (keep comment for code generation)

	// URL formatting
	joined, err := url.JoinPath(c.coreClient.BaseURL, "/v1/"+sdkcore.FmtStringParam(request.Parent)+"/analytics/"+"datastores")
	if err != nil {
		return types.GoogleCloudApigeeV1Datastore{}, err
	}
	...
	if resp.StatusCode >= 300 {
		return types.GoogleCloudApigeeV1Datastore{}, sdkcore.NewApiError(*req, *resp, body)
	}
	var bodyData types.GoogleCloudApigeeV1Datastore
...
// List Datastores
func (c *Client) List(request ListRequest, reqModifiers ...RequestModifier) (types.GoogleCloudApigeeV1ListDatastoresResponse, error) {
	// build & send request (keep comment for code generation)

	// URL formatting
	joined, err := url.JoinPath(c.coreClient.BaseURL, "/v1/"+sdkcore.FmtStringParam(request.Parent)+"/analytics/"+"datastores")
	if err != nil {
		return types.GoogleCloudApigeeV1ListDatastoresResponse{}, err
	}
	url, err := url.Parse(joined)
	if err != nil {
		return types.GoogleCloudApigeeV1ListDatastoresResponse{}, err
	...
	targetType, err := request.TargetType.Value()
	if err != nil {
		sdkcore.AddQueryParam(params, "targetType", targetType, false)
	}
	uploadType, err := request.UploadType.Value()
	if err != nil {
		sdkcore.AddQueryParam(params, "uploadType", uploadType, false)
	...
	// Check status
	if resp.StatusCode >= 300 {
		return types.GoogleCloudApigeeV1ListDatastoresResponse{}, sdkcore.NewApiError(*req, *resp, body)
	}
	var bodyData types.GoogleCloudApigeeV1ListDatastoresResponse
	err = json.Unmarshal(body, &bodyData)
	if err != nil {
		return types.GoogleCloudApigeeV1ListDatastoresResponse{}, err
	}
