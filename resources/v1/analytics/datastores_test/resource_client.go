...
func (c *Client) Create(request CreateRequest, reqModifiers ...RequestModifier) (types.GoogleCloudApigeeV1TestDatastoreResponse, error) {
	// build & send request (keep comment for code generation)

	// URL formatting
	joined, err := url.JoinPath(c.coreClient.BaseURL, "/v1/"+sdkcore.FmtStringParam(request.Parent)+"/analytics/"+"datastores:test")
	if err != nil {
		return types.GoogleCloudApigeeV1TestDatastoreResponse{}, err
	}
	...
	if resp.StatusCode >= 300 {
		return types.GoogleCloudApigeeV1TestDatastoreResponse{}, sdkcore.NewApiError(*req, *resp, body)
	}
	var bodyData types.GoogleCloudApigeeV1TestDatastoreResponse
