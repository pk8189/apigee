...
func (c *Client) Create(request CreateRequest, reqModifiers ...RequestModifier) (types.GoogleCloudApigeeV1DeveloperBalance, error) {
	// build & send request (keep comment for code generation)

	// URL formatting
	joined, err := url.JoinPath(c.coreClient.BaseURL, "/v1/"+sdkcore.FmtStringParam(request.Name)+":credit")
	if err != nil {
		return types.GoogleCloudApigeeV1DeveloperBalance{}, err
	}
	...
	if resp.StatusCode >= 300 {
		return types.GoogleCloudApigeeV1DeveloperBalance{}, sdkcore.NewApiError(*req, *resp, body)
	}
	var bodyData types.GoogleCloudApigeeV1DeveloperBalance
