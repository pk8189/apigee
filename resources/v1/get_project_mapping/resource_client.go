...
func (c *Client) List(request ListRequest, reqModifiers ...RequestModifier) (types.GoogleCloudApigeeV1OrganizationProjectMapping, error) {
	// build & send request (keep comment for code generation)

	// URL formatting
	joined, err := url.JoinPath(c.coreClient.BaseURL, "/v1/"+sdkcore.FmtStringParam(request.Name)+":getProjectMapping")
	if err != nil {
		return types.GoogleCloudApigeeV1OrganizationProjectMapping{}, err
	}
	...
	if resp.StatusCode >= 300 {
		return types.GoogleCloudApigeeV1OrganizationProjectMapping{}, sdkcore.NewApiError(*req, *resp, body)
	}
	var bodyData types.GoogleCloudApigeeV1OrganizationProjectMapping
