...
func (c *Client) List(request ListRequest, reqModifiers ...RequestModifier) (types.GoogleLongrunningListOperationsResponse, error) {
	// build & send request (keep comment for code generation)

	// URL formatting
	joined, err := url.JoinPath(c.coreClient.BaseURL, "/v1/"+sdkcore.FmtStringParam(request.Name)+"/operations")
	if err != nil {
		return types.GoogleLongrunningListOperationsResponse{}, err
	}
	...
	if resp.StatusCode >= 300 {
		return types.GoogleLongrunningListOperationsResponse{}, sdkcore.NewApiError(*req, *resp, body)
	}
	var bodyData types.GoogleLongrunningListOperationsResponse
