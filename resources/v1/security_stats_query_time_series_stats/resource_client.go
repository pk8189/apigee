...
package security_stats_query_time_series_stats
...
func (c *Client) Create(request CreateRequest, reqModifiers ...RequestModifier) (types.GoogleCloudApigeeV1QueryTimeSeriesStatsResponse, error) {
	// build & send request (keep comment for code generation)

	// URL formatting
	joined, err := url.JoinPath(c.coreClient.BaseURL, "/v1/"+sdkcore.FmtStringParam(request.Orgenv)+"/securityStats:queryTimeSeriesStats")
	if err != nil {
		return types.GoogleCloudApigeeV1QueryTimeSeriesStatsResponse{}, err
	}
	...
	if resp.StatusCode >= 300 {
		return types.GoogleCloudApigeeV1QueryTimeSeriesStatsResponse{}, sdkcore.NewApiError(*req, *resp, body)
	}
	var bodyData types.GoogleCloudApigeeV1QueryTimeSeriesStatsResponse
